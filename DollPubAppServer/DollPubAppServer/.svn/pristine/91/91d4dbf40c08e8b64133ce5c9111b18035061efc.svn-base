...
import com.imlianai.dollpub.app.modules.support.exchange.dao.DollComposeDao;
import com.imlianai.dollpub.app.modules.support.exchange.dao.DollExchangeDao;
import com.imlianai.dollpub.app.modules.support.exchange.enm.DebrisTypeEnm;
...
public class DollComposeServiceImpl implements DollComposeService {
		...
		if(!StringUtil.isNullOrEmpty(composeDolls)) {
			for(DollInfo dollInfo : composeDolls) {
				...
				ComposeDollInfo cDollInfo = new ComposeDollInfo(dollInfo.getDollId(), dollInfo.getName(), dollInfo.getImgCompose(), 
						dollInfo.getInventory()-exchangeNum, dollInfo.getCommDebrisNum(), dollInfo.getRareDebrisNum(), dollInfo.getImgRoom());
	...
	public BaseRespVO compose(Long uid, int dollId) {
		try {
			...
			if(dollInfo.getInventory() == 0) {
				return new BaseRespVO(-1, false, "该娃娃库存为0，不可合成~");
			}
