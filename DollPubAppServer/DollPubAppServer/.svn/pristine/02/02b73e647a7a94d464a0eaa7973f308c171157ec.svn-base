...
public class CustomerDollBusServiceImpl implements CustomerDollBusService {
    ...
    public DollBusApplyMachineRespVO applyMachine(long uid, int busId, Customer customer, String remark,int type) {
        ...
        if (customer != null) {
            ...
            if (dollBusStatus == null || dollBusStatus.getStatus() == MachineStateConst.LEISURE) {
                ...
                return this.apply(uid, busId, customer, remark,type);
            }
        ...
        if (optRecord != null) {
            ...
            try {
                ...
                if (customer != null && customer.getLevel() == 1) {
                	...
                	if (customer!=null&&customer.getId()==93) {
                		DollBus dollBus = dollBusService.getDollBus(busId);
                		dollService.handleH5ResultMsgOnly(dollBus, optRecord, catchResult, groupId);
					}
                    if (busStatusDAO.deleteDollBusStatusByBusId(busId) > 0) {
                        logger.info("清除上机状态成功=>" + busId);
                    }
     ...
     * @return
     */
    private DollBusApplyMachineRespVO apply(long uid, int busId, Customer customer, String remark,int type) {
        DollBus dollBus = dollBusService.getDollBus(busId);
        if (dollBus != null) {
            MachineOpt machineOpt;
            if (type != 0){
                logger.info("新版子上机 类型=>" + type);
                machineOpt = iMachineRemoteService.applyMachineType(uid, busId, customer.getGroupId(),type);
            }else {
                machineOpt = iMachineRemoteService.applyMachine(uid, busId, customer.getGroupId());
            }
            ...
            if (machineOpt != null) {
                ...
                try {
                    ...
                    if (userBase != null){
                        optRecord.setAgentId(userBase.getAgentId());
                    }else {
                        optRecord.setAgentId(0);
                        optRecord.setAgentName("");
                    }
