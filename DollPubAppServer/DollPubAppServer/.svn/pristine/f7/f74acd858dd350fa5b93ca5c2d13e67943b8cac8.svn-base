...
public class ShopkeeperInfoDaoImpl implements ShopkeeperInfoDao {
    ...
    public int init(ShopkeeperInfo info) {
        return jdbcHandler.executeSql("INSERT INTO shopkeeper_info(uid,name,number,openId,ratioId,type,totalInviteNum,totalIncomeMoney,totalWithdrawMoney,maxWithdrawMoney,minWithdrawMoney,totalWithdrawDay,withdrawFn,status,createTime) " +
                "VALUES(?,?,?,?,1,0,0,0,0,1000,10,3,1,0,NOW())",info.getUid(),info.getName(),info.getNumber(),info.getOpenId());
    }
    ...
    @Override
    public int updateStatus(int status,long uid) {
        return jdbcHandler.executeSql("UPDATE shopkeeper_info SET `status`=? WHERE uid=? AND `status`<>?",status,uid,status);
    }
