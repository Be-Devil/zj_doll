...
import com.imlianai.dollpub.constants.ChannelConstants;
...
public class RandomImageGenerator {
	...
	private static Font getFont() {
		...
		font[1] = new Font("Antique Olive Compact", Font.BOLD, 40);
...
//		 return font[random.nextInt(5)];
	 ...
	 * @throws IOException
	 */
	public static void render(String randomStr, HttpServletResponse response, int width,
			int height) throws IOException {
		render(randomStr, response.getOutputStream(), width, height);
	}
	 ...
	 * @param numberStr
	 */
	public static String getCheckCodeUrl(String numberStr){
		if (!AppUtils.isTestEnv()) {
			return "http://www.mengquww.com/api/userVerification/code?key="+numberStr+"&uuid="+JUUIDUtil.createUuid();
		}else{
			return "http://122.11.48.251:6639/DollAppServer/api/userVerification/code?key="+numberStr+"&uuid="+JUUIDUtil.createUuid();
		}
	}
}
