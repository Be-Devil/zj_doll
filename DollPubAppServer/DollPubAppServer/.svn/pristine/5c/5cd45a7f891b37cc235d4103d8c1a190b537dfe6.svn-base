...
public class TradeRecordDAOImpl implements TradeRecordDAO{
	...
	private String addRecord = "insert into trade_record(uid,tid,type,recordType,code,cost,costType,remark,time) values(?,?,?,?,?,?,?,?,now()) ";
	@Override
	public long addRecord(TradeRecord record) throws TradeOperationException {
		try {
			return jdbcHandler.executeSql(addRecord, record.getUid(), record.getTid(), record.getType(), record.getRecordType(),
					record.getCode(), record.getCost(), record.getCostType(),record.getRemark());
		} catch (Exception e) {
	...
	private String getRecordsAll = "select * from trade_record where uid=? order by id desc ";
	@Override
	public List<TradeRecord> getRecords(long uid, int page, int limit) {
		try {
			if (page==0) {
				return jdbcHandler.queryBySql(TradeRecord.class, getRecordsAll, uid);
			}
