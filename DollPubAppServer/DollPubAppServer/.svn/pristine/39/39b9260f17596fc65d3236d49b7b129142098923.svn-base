package com.imlianai.dollpub.app.modules.support.banner.dao;
...
public class BannerDAOImpl implements BannerDAO {
	...
	private String getBanners = "select * from banner where customerId=? and  valid=1 and endTime>now() and startTime<now()"
			+ " order by `index` asc ";

	@Override
	public List<Banner> getBanners(int location,int customerId) {
		return jdbcHandler.queryBySql(Banner.class, getBanners,customerId);
	}
	...
	public String getVideourl() {
		return jdbcHandler.queryOneBySql(String.class, "select url from banner_video_url limit 1");
	}
