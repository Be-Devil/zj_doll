...
public class DiceDaoImpl implements DiceDao {
    ...
    @Override
    public List<DiceStatus> getTimeOutRecord(int timeOut) {
        return jdbcHandler.queryBySql(DiceStatus.class,"select * from dice_status where createTime < DATE_SUB(NOW(),INTERVAL ? SECOND)",timeOut);
    }
}
