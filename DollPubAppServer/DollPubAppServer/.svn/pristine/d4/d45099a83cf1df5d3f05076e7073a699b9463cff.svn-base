...
import com.imlianai.dollpub.app.modules.publics.oauth.merchant.service.MerchantOauthService;
...
public class CmdShopkeeper extends RootCmd {
	...
	public BaseRespVO rank(BaseReqVO vo) {
		...
		List<ShopkeeperDayRankDTO> realList = shopkeeperService
				.getShopkeeperDayRank();
		...
		if (!StringUtil.isNullOrEmpty(rank)) {
			for (DollShopRobot dollShopRobot : rank) {
				if (matain > 0 && dollShopRobot.getAmt() > 0) {
					UserGeneral user = userService.getUserGeneral(dollShopRobot
							.getUid());
					if (user != null) {
						dtoList.add(new ShopkeeperDayRankDTO(user.getName(),
								user.getHead(), dollShopRobot.getAmt() / 100));
						matain--;
					}
				}
			}
	...
	public BaseRespVO getWithdrawRecord(BaseReqVO vo) {
		List<ShopkeeperWithdrawRecord> list = shopkeeperService
				.getWithdrawRecord(vo.getUid());
	...
	public BaseRespVO hasInitShop(BaseReqVO vo) {
		...
		ShopkeeperQRCode qrcode = shopkeeperService.getShopkeeperQRCode(vo
				.getUid());
		Map<String, Object> map = new HashMap<String, Object>();
	...
	@ApiHandle
	@Path("api/shopkeeper/bindAppWechat")
	public BaseRespVO bindAppWechat(String openId, String unionId,
			String userId, String pwd, int authCustomerId) {
		long uid = merchantOauthService.loginWithSrcPwd(authCustomerId, userId,
				pwd);
		if (uid > 0) {
			UserBase user=userService.getUserBase(uid);
			if (user!=null&&user.getSrcType()==UserSrcType.SRC_APP.type) {
				userService.updateWechatBound(uid, unionId, openId);
				userService.updateUserFollow(uid);
			}
			return new BaseRespVO();
		}
		return new BaseRespVO(0, false, "账号密码错误");
