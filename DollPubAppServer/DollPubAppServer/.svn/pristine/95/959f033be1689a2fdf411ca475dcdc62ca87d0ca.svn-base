...
public class ShopkeeperServiceImpl implements ShopkeeperService {
    ...
    public void initShopkeeper(long uid, long inviteUid) {
        ...
        if (userBase != null) {
            if (userBase.getCustomerId() == 84){
                //判断是否为店主
                if (!this.isShopkeeper(uid)) {
                    //创建店主
                    UserGeneral userGeneral = userService.getUserGeneral(uid);
                    if (userGeneral != null){
                        int flag = shopkeeperInfoDao.init(new ShopkeeperInfo(uid,userGeneral.getName(),userBase.getNumber(),userBase.getSrcId()));
                        if (flag > 0) {
                            //处理邀请关系
    ...
    public ShopkeeperInfo getShopkeeperInfo(long uid){
    	...
    	if (info==null) {
			initShopkeeper(uid, 0);
			info=shopkeeperInfoDao.getInfoByUid(uid);
		}
    	return info;
    }
    ...
    public BaseRespVO getShopkeeperWalletInfo(long uid) {
        ShopkeeperInfo info = getShopkeeperInfo(uid);
        if (info != null) {
            try{
    ...
    public BaseRespVO applyWithdraw(long uid, int money) {
        ...
        if (userBase != null) {
            ...
            // 验证实名信息绑定
            if(withdrawDao.hasCertificationInfo(uid)==0){
            	return new BaseRespVO(103, false, "未实名");
            } 
            //微信授权绑定
            if (userBase.getSrcType()==UserSrcType.SRC_APP.type) {
				if(StringUtil.isNullOrEmpty(userBase.getSrcId())||StringUtil.isNullOrEmpty(userBase.getSrcUnionId())){
					return new BaseRespVO(104, false, "完成服务");
				}
			}
            //验证关注公众号
            ...
            if (info != null) {
                ...
                if (money < info.getMinWithdrawMoney() * 100)
                    return new BaseRespVO(0, false, "单笔提现金额不能少于"+info.getMinWithdrawMoney()+"元");
	...
	public int updatePayState(long id, int payState,int status) {
		return shopkeeperWithdrawRecordDao.updatePayState(id, payState,status);
	}

	@Override
	public int updateReason(long id, String remark,String wechatPaymentId) {
		return shopkeeperWithdrawRecordDao.updateReason(id, remark,wechatPaymentId);
	}
