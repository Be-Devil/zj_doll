    ...
    function request(paramName) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                var commDebrisNum = datamsg.commDebrisNum;
                var id = datamsg.id;
                var name = datamsg.name;
                var path = datamsg.path;
                var rareDebrisNum = datamsg.rareDebrisNum;
                ...
                var html = '<section class="patchList comList" id="'+id+'"><div class="patchList_pic comList_pic">'+
                    ...
                    '<p class="p_need c_need">需要：<img src="http://lianai-image-sys.qiniudn.com/mall/pic_3.png" alt=""/><b>'+commDebrisNum+'</b>&nbsp;+&nbsp;<img src="http://lianai-image-sys.qiniudn.com/mall/pic_4.png" alt=""/><b>'+rareDebrisNum+'</b></p></div></section>'+
                    '<section class="com_pic">'+
                    '<h6>娃娃详情</h6>'+
                    '<img src="'+imgRoom+'" alt=""/></section>'
                $('.particulars').append(html);
                //合成按钮
                var btnAlert ='<div class="com_btn" ><button class="com_btn1"  >合成</button></div>'
                //添加到particulars前
                ...
                $('.com_btn1').click(function () {
                    //渲染
                    var htmlMsater = ' <div class="overlay" style="display: none"></div><div class="waDhPop" style="display: none"> <p class="new_p">确认消耗<em>'+commDebrisNum+'</em>块普通娃娃碎片和<em>'+rareDebrisNum+'</em>块稀有娃娃碎片合成<em>'+name +'</em>?</p > <span class="buttonBox ks-clear"> <i class="dhCancle">取消</i> <i class="dhConfirm">确定</i> </span></div>';
                    ...
                    $('.dhConfirm').click(function (){
                        //判断用户的普通碎片和稀有碎片
                        $.ajax({
                            url: baseUrl + '/api/dollCompose/compose',
                            type: 'post',
                            //cache:true,
                            data: JSON.stringify(
                                {
