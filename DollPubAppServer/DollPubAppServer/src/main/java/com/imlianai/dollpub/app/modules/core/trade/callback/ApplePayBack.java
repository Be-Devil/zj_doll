package com.imlianai.dollpub.app.modules.core.trade.callback;
...
import com.imlianai.dollpub.app.modules.core.trade.callback.service.PayBackExcuteService;
...
import com.imlianai.dollpub.app.modules.core.trade.vo.ApplePayBackReqVO;
	...
	private int handleChargeBill(List<Object> inAppList, ApplePayBackReqVO vo,
		...
		if (inAppList != null && !inAppList.isEmpty()) {
			...
			for (Object object : inAppList) {
				...
				ChargeCatalog catalog = chargeCatalogService.getCatalog(code);
				if (catalog != null) {
					charge = new TradeCharge(vo.getUid(), ChargeWay.APPLE,
							code, catalog.getPrice(), vo.getChannel(),
							vo.getOsType(),vo.getImei(),  vo.getCustomerId(),vo.getCustomerId());
					charge.setOtherId(tradeId);
					long id = tradeChargeService.add(charge);
					if (id > 0) {// 订单插入成功才继续后续操作，不成功则需要将返回map置为false
						tradeChargeService.addLog(id, vo.getUid(),
								vo.getTransactionReceipt(), requestResult);
