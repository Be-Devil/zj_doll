...
public static void main(String[] args) {
	String url="http://122.11.48.251:6636/LiveAppServer/call.do?cmd=user.srcoauth&action=queryInfo";
	//String url="http://www.nuannuanzhibo.com/call.do?cmd=user.srcoauth&action=queryInfo";
	Map<String, Object> postDate=new HashMap<String, Object>();
	Map<String, String> headDate=new HashMap<String, String>();
	headDate.put("oauth", "oauth");
	
	postDate.put("userId", 1444967);
	postDate.put("token", "189590387c7f444d99fc8054b4a1dc4f");
	postDate.put("action", "queryInfo");
	HttpEntity httpEntity =HttpUtil.Post(url, JSON.toJSONString(postDate),headDate);
	System.out.println(httpEntity.getHtml());
}
	@Override
	public long initUser(int customerId,int authCustomerId, String srcUid, String token) {
		//查询系统表是否存在
		logger.info("initUser customerId:"+customerId+" authCustomerId:"+authCustomerId+" srcUid:"+srcUid+" token:"+token);
		CustomerAuthToken authToken=userSrcDao.getCustomerAuthToken(authCustomerId, srcUid);
		logger.info("initUser authToken:"+JSON.toJSONString(authToken));
		userSrcDao.addCustomerAuthToken(authCustomerId, srcUid, token);
		Customer customer=customerService.getCustomerById(authCustomerId);
		String url=customer.getOauthUrl();
		Map<String, Object> postDate=new HashMap<String, Object>();
		postDate.put("userId", srcUid);
		...
		if (httpEntity!=null) {
			if (!StringUtil.isNullOrEmpty(httpEntity.getHtml())) {
				...
				logger.info("initUser resp:"+httpEntity.getHtml());
				if (resp!=null) {
					if (resp.isState()) {
						Object uObject= resp.getData();
						logger.info("initUser uObject:"+JSON.toJSONString(uObject));
						CustomerAuthUserInfo info=JSON.parseObject(JSON.toJSONString(uObject), CustomerAuthUserInfo.class);
						if (info!=null) {
							UserBase userBase = new UserBase(0, null);
							userBase.setCustomerId(customerId);
							userBase.setAgentId((long)authCustomerId);
							userBase.setSrcType(UserSrcType.SRC_APP.type);
							UserGeneral user = new UserGeneral();
							user.setName(info.getName());
							user.setHead(info.getHead());
							user.setCustomerId(customerId);
							user.setAgentId((long)authCustomerId);
							if (authToken==null||authToken.getInnerUid()==0) {
								long uid=initUser(userBase, user);
								if (uid>0) {
									userSrcDao.updateInnerUid(authCustomerId, srcUid, uid);
									return uid;
								}
							}else{
								UserUpdateReqVO userUpdate=new UserUpdateReqVO();
								userUpdate.setUid(authToken.getInnerUid());
								userUpdate.setHead(info.getHead());
								userUpdate.setName(info.getName());
								updateUserInfo(userUpdate);
								return authToken.getInnerUid();
							}
						}
					}
				}
	...
	@Override
	public UserBase getUserBaseByOauthCustomerSrcId(String srcId, int customerId) {
		long uid=userSrcService.getCustomerAuthUser(srcId, customerId);
		if (uid>0) {
			return getUserBase(uid);
		}
		return null;
	}

	@Override
	public void updateWechatBound(long uid, String unionId, String openId) {
		userDAO.updateSrcIdUnionId(uid, openId, unionId);
	}
}
