...
public class CustomerOptRecordDaoImpl implements CustomerOptRecordDao {
	...
	private String insert_sql = "insert into %s(optId,busId,dollId,uid,customerId,agentId,agentName,startTime,endTime,result,remark) values(?,?,?,?,?,?,?,now(),?,?,?)";
	...
	public int insertEntity(int groupId,OptRecord optRecord) {
		try {
			return jdbcHandler.executeSql(cusOptRecordRouter.getFormatSql(insert_sql, groupId),
					optRecord.getOptId(), optRecord.getBusId(), optRecord.getDollId(),optRecord.getUid(), optRecord.getCustomerId(),optRecord.getAgentId(),optRecord.getAgentName(),
