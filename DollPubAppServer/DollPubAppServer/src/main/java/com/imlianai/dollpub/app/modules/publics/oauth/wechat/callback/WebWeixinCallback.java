	...
	public String doBack(HttpServletRequest request,
		...
		case 1: {// 文字内容信息
			...
			if (StringUtils.isNotBlank(content)) {
				if (content.indexOf("暖暖红包提现") >= 0) {
					backStr="暖暖用户您好，完成红包提现绑定，请点击<a href='http://www.realgamecloud.com/api/oauth/wxAuth/84/appbindwechat?appid=wx85cf26e3d3d976ec&userInfoMark=userInfoMark&agentId=2&uy=1'>账号绑定</a>";
		...
		case 4: {// click
			...
			if (event.trim().equals("consult")) {
				...
				backStr =WebWeixinXmlUtils.setBackPicMsgNoCDATA(openId, publicAccountID, "G1mPr5vYZQ9vKaayEPTMYpQBFh3VBlaixTQ-6JxupVc");
			}
		}
			break;
		case 5: {// 订阅消息推送
		...
		UserBase userBase=userService.getUserBaseBySrc(UserSrcType.WECHAT_PUBLIC.type, openId, null, 84);
		if (userBase!=null) {
			userService.updateUserFollow(userBase.getUid());
		}
		logger.info("back_info:" + backStr);
		PrintWriter pw = response.getWriter();
		pw.print(backStr);
