...
public class PushCoinVirtualOptRouterDaoImpl implements PushCoinVirtualOptRouterDao {
    ...
    public int updateSlots(int groupId, long optId, int slots) {
        return jdbcHandler.executeSql(pushCoinUnityOptRouter.getFormatSql("update %s",groupId) + " set slots=slots+?,slotsTime=slotsTime+1,endTime=now() where state<>1 and optId=?",slots,optId);
    }
}
