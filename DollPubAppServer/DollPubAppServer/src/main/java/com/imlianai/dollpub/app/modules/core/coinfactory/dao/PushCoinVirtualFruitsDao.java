...
public interface PushCoinVirtualFruitsDao {
     ...
     * @return
     */
    PushCoinFruitsWin getRecordByOne();
     ...
     * @return
     */
    int updateAllotResult(long id,int result,String remark);
    ...
    PushCoinFruitsAllot getOneWinAllotById(long id);

    PushCoinFruitsAllot getOneWinAllotByCondition(int busId,int customerId,long uid);

    /**
     * 更新确认中奖结果
     * @param id
     * @param actual
     * @param dialRate 转盘实际倍率
     * @param optId
     * @param remark
     * @return
     */
    int updateAllotAffirmResult(long id,int actual,int dialRate,long optId, String remark);
