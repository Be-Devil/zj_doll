...
public class DollServiceImpl implements DollService {
	...
	@Override
	public void handleH5ResultMsgOnly(DollBus bus, OptRecord optRecord,
			boolean isSuccess, int group) {
		if (optRecord != null) {
			...
			if (isSuccess) {
				...
				userGeneral.setUid(optRecord.getUid());
				MsgRoom msgRoom = new MsgRoom(bus, userGeneral,
						MsgRoomType.GAIN_SUCCESS.type, "捉取成功");
				msgService.sendMsgRoom(msgRoom);
			} else {
				UserGeneral userGeneral =new UserGeneral();
				userGeneral.setUid(optRecord.getUid());
				MsgRoom msgRoom = new MsgRoom(bus, userGeneral,
						MsgRoomType.GAIN_FAIL.type, "捉取失败");
				msgService.sendMsgRoom(msgRoom);
			}
		}
	}
