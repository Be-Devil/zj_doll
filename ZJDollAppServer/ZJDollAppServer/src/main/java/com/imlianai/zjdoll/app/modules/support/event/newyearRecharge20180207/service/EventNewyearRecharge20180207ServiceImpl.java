package com.imlianai.zjdoll.app.modules.support.event.newyearRecharge20180207.service;
...
import com.imlianai.zjdoll.app.modules.support.event.newyearRecharge20180207.dao.EventNewyearRecharge20180207Dao;
...
import com.imlianai.zjdoll.app.modules.support.event.newyearRecharge20180207.utils.NewyearRechargeUtils;
import com.imlianai.zjdoll.app.modules.support.event.newyearRecharge20180207.vo.BaseUserInfo;
...
public class EventNewyearRecharge20180207ServiceImpl implements
	...
	public static String EVENT_URL = PropertiesUtil.getString("application","newYearEvent.url"); // 新春充值活动h5地址
	...
	public void handleRecharge(double cost, long uid) {
		try {
			if (NewyearRechargeUtils.eventStatus() == 1) { // 活动期间
	...
	public void newYearEventEndPushMsg() {
		...
		if (!StringUtil.isNullOrEmpty(userBlessingList)) {
			...
			for (Event20180207NewyearRechargeUserBlessing userBlessing : userBlessingList) {
				Long uid = userBlessing.getUid();
				String body = "#缤纷夏日，充值送礼  恭喜小主获得贵人榜第" + ranking
	...
	public void handleMsgTask() {
		...
		if (msg1) {
			msgRoomJump = new MsgRoomJump(null, MsgRoomType.BUS_SYS.type,
					"春暖花开，充值送礼，贵人榜单前10名可获美图M8s、美的洗碗机、美的扫地机器人等丰厚礼品！>");
			msgRoomJump
					.setDefaultStyleText("春暖花开，充值送礼，贵人榜单前10名可获美图M8s、美的洗碗机、美的扫地机器人等丰厚礼品！>");
			msgRoomJump.setJumpUrl(EVENT_URL);
			msgRoomJump.setTargetTitle("春暖花开，充值送礼");
			msgRoomJump.setSave(true);
			msgService.sendMsgRoomAll(msgRoomJump);
		}
	}
