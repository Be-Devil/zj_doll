...
public class VersionDAOImpl implements VersionDAO {
	...
	public String getBGM() {
		return jdbcHandler.queryOneBySql(String.class, "select url from background_music where startTime<=now() and endTime>now()");
	}

	@Override
	public void updateDeviceInfo(String imei,String channel,int osType) {
		jdbcHandler.executeSql("insert into device_imei (imei,createTime,createDate,updateTime,updateDate,channel,osType) value (?,now(),DATE_FORMAT(now(),'%Y-%m-%d'),now(),DATE_FORMAT(now(),'%Y-%m-%d'),?,?) on duplicate key update updateTime=now(),updateDate=DATE_FORMAT(now(),'%Y-%m-%d')", imei,channel,osType);
	}
}
