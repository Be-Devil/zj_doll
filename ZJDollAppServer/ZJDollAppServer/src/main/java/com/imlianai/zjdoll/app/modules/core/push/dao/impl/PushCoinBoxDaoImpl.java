...
public class PushCoinBoxDaoImpl implements PushCoinBoxDao {
    ...
    @Override
    public int initBox(int busId, int price) {
        return jdbcHandler.executeSql("insert into push_coin_box(busId,total,createTime) value(?,base*?,now())",busId,price);
    }

    @Override
    public int updateTotal(int busId, int price) {
        return jdbcHandler.executeSql("update push_coin_box set total=base*?,updateTime=now() where busId=?",price,busId);
    }
    ...
    @Override
    public List<PushCoinBox> getBoxListByBusIds(List<Integer> busIds) {
        return null;
    }
    ...
    public int addOpenBoxRecord(PushCoinBoxLog boxLog) {
        return jdbcHandler.executeSql("insert into push_coin_box_log(uid,busId,boxId,type,num,rate,lastId,remark,virtual,createTime) values(?,?,?,?,?,?,?,?,?,now())",
                boxLog.getUid(),boxLog.getBusId(),boxLog.getBoxId(),boxLog.getType(),boxLog.getNum(),boxLog.getRate(),boxLog.getLastId(),boxLog.getRemark(),boxLog.getVirtual());
    }
