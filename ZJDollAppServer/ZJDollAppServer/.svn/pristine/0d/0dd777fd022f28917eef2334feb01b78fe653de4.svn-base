...
public class DollBusServiceImpl implements DollBusService {
		...
		if (!StringUtil.isNullOrEmpty(list2)) {
			for (ZengjingDollBusDTO zengjingDollBusDTO : list2) {
				bus.add(new DollBus(zengjingDollBusDTO.getBusId()+"",
						...
						zengjingDollBusDTO.getStream1(), zengjingDollBusDTO.getStream2(), zengjingDollBusDTO.getStream1Realy(), zengjingDollBusDTO.getStream2Realy(),zengjingDollBusDTO.getName(),zengjingDollBusDTO.getType(),zengjingDollBusDTO.getVirtual()));
			}
		}
		return bus;
	}
	...
	public List<DollBus> getAllDollBus() {
		return dollBusDao.getAllDollBus();
	}
