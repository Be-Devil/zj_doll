...
public class PushCoinOptRouterDaoImpl implements PushCoinOptRouterDao {
    ...
    public long insert(MachinePushCoin pushCoin) {
        String insert = "insert into push_coin_opt_88 (optId,deviceId,busId,uid,customerId,price,rate,time,prizeType,startTime,endTime,createTime) " +
                "values(?,?,?,?,?,?,?,?,?,now(),date_add(now(),interval ? second),now())";
        return jdbcHandler.executeSql(insert,pushCoin.getOptId(),pushCoin.getDeviceId(),pushCoin.getBusId(),pushCoin.getUid(),pushCoin.getCustomerId(),pushCoin.getPrice(),pushCoin.getRate(),pushCoin.getTime(),
                pushCoin.getPrizeType(),pushCoin.getTime());
    }
    ...
    public List<MachinePushCoin> getTimeOutRecord() {
        return jdbcHandler.queryBySql(MachinePushCoin.class,"select * from push_coin_opt_88 where endTime < date_sub(now(),interval time second) and state<>1");
    }
    ...
    @Override
    public List<MachinePushCoin> getUserPlayRecordList() {
        return jdbcHandler.queryBySql(MachinePushCoin.class,"select * from push_coin_opt_88 where state<>1");
    }
    ...
    @Override
    public int updateCoin(int coin, long optId) {
        return jdbcHandler.executeSql("update push_coin_opt_88 set coin=coin+?,endTime=date_add(now(),interval time second) where jewelStatus<>1 and state<>1 and optId=?",coin,optId);
    }
