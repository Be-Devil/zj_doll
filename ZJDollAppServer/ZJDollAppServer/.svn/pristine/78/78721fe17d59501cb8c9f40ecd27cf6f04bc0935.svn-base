...
public class TradeDAOImpl implements TradeDAO{
	...
	String updateCoupon = "update trade_account set coupon=coupon+?,time=now() where uid=? and coupon+?>=0";
	@Override
	public int updateCoupon(long uid, int coupon) {
		try {
			return jdbcHandler.executeSql(updateCoupon, coupon, uid, coupon);
		} catch (Exception e) {
			logger.error("TradeDAO.updateCoupon error:"+e);
			return -1;
		}
	}
}
